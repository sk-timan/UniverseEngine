...
（无 — 主规范目录尚无 level-persistence 规范；本文件定义关卡对 uasset 的引用与加载约束。）

## 移除需求

### 需求:Level 通过源文件路径 Assimp 回退导入

**Reason**: 由引擎资产 Load 取代；源文件仅用于 Editor Import/Reimport。

**Migration**: 现有关卡需重新 Import 模型生成 uasset，或运行迁移工具将 mesh_asset_id 更新为 SoftObjectPath。
