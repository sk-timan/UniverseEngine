...
定义 Editor Content Browser 面板的布局、浏览、过滤、拖拽与右键交互，对齐 UE5 风格资产浏览工作流。
...
### 需求:Content Browser 必须支持外部资源拖入导入

系统必须允许用户将外部 3D 模型文件拖放到 Content Browser 当前选中文件夹的资产网格区域，并触发与菜单「导入」等价的资产写入流程。

#### 场景:拖入支持的模型格式

- **当** 用户在左侧选中某一文件夹（非 `All`），并将外部文件拖放到右侧资产网格区域
- **当** 拖入文件为支持的 3D 模型格式（如 fbx、obj、gltf、glb、dae、3ds、blend）
- **那么** 系统必须调用 `GameApp::ImportAssetFromSourceFile`（或等价导入流程）将资产写入该文件夹对应的 Content 虚拟路径
- **那么** 导入成功后 Content Browser 必须刷新并显示新资产 Tile

#### 场景:目标路径规则

- **当** 当前选中文件夹为 `Content`
- **那么** 导入资产的 AssetPath 必须为 `<ObjectName>`

- **当** 当前选中文件夹为 `Meshes/Import` 等子路径
- **那么** 导入资产的 AssetPath 必须为 `<FolderPath>/<ObjectName>`

#### 场景:无效拖放必须被拒绝

- **当** 当前选中 `All` 根节点
- **那么** 系统必须拒绝拖放并提示用户选择具体文件夹

- **当** 拖入非支持格式文件
- **那么** 系统必须拒绝拖放或提示不支持的文件类型
